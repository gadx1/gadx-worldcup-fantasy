...
export function SurvivorBracketPanel({
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span
            className={`truncate text-sm ${
              isWinner ? 'font-bold text-[var(--ink-900)]' : 'font-medium text-[var(--ink-900)]'
            }`}
          >
            {team?.countryName ?? teamId}
          </span>
        </div>
        <span className="shrink-0 font-display text-sm font-bold text-[var(--ink-900)]">
          {score}
        </span>
      </div>
    )
  ...
  return (
    <article className="rounded-3xl border border-[var(--pitch-900)]/10 bg-white/85 p-6 shadow-sm sm:p-8">
      ...
      {assignments.length > 0 && (
        <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2">
          {sortedPlayers.map((player) => (
            <span key={player.id} className="inline-flex items-center gap-2 text-xs text-[var(--ink-600)]">
              <span
      ...
      {!hasAnyMatches ? (
        <div className="mt-6 rounded-2xl border border-dashed border-[var(--pitch-900)]/20 bg-[var(--bone-50)] p-8 text-center">
          <p className="font-display text-lg font-bold uppercase text-[var(--ink-900)]">
            Bracket not set yet
          </p>
          ...
          <div className="flex min-w-max gap-4">
            {columns.map((column) => (
              <div key={column.key} className="flex w-60 shrink-0 flex-col">
                <p className="mb-3 text-center text-xs font-semibold uppercase tracking-[0.2em] text-[var(--pitch-700)]">
                  {column.label}
                </p>
                {column.matches.length === 0 ? (
                  <div className="flex flex-1 items-center justify-center rounded-2xl border border-dashed border-[var(--pitch-900)]/15 p-6 text-center text-xs text-[var(--ink-600)]">
                    To be decided
                  </div>
                ) : (
                  <div className="flex flex-1 flex-col justify-around gap-4">
                    {column.matches.map((match) => {
                      ...
                      return (
                        <div
                          key={match.id}
                          className="overflow-hidden rounded-2xl border border-[var(--pitch-900)]/12 bg-white shadow-sm"
                        >
                          <TeamRow
                          ...
                          />
                          <div className="h-px bg-[var(--pitch-900)]/10" />
                          <TeamRow
                          ...
                          />
                          {!played && (
                            <p className="bg-[var(--bone-50)] px-3 py-1 text-center text-[0.65rem] uppercase tracking-wide text-[var(--ink-600)]">
                              {match.status}
                            </p>
                          )}
                        </div>
